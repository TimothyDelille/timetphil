const ArrowDrawing = () => {
    const stroke = "#ffc067";
    return (
    <svg version="1.1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 94.06113603136941 95.60964287493192">
        <path d="M0.06 1.83 C7.33 6.23, 29.92 12.63, 41.7 24.54 C53.47 36.45, 66.56 65.03, 70.72 73.27 M-3.34 0.35 C4.97 5.23, 34.77 14.53, 46.99 27.13 C59.2 39.73, 66.78 67.71, 69.97 75.96" stroke={stroke} strokeWidth="4" fill="none"></path>
        <path d="M55.54 55.06 C56.41 62.45, 60.89 64.09, 69.94 75.15 M53.89 55.58 C59.25 63.7, 65.08 70.11, 70.31 75.9" stroke={stroke} strokeWidth="4" fill="none"></path>
        <path d="M71.71 49.49 C68.77 58.19, 69.48 61.12, 69.94 75.15 M70.06 50.02 C69.72 60.22, 69.82 68.59, 70.31 75.9" stroke={stroke} strokeWidth="4" fill="none"></path>
    </svg>
    )
  }
export default ArrowDrawing;